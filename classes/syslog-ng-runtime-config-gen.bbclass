...
python update_constants () {
    ...
    with open(config_file, 'w') as conf:
        ...
        conf.write("destination d_dcm_bridge {\n")
        conf.write("    program(\"/usr/bin/dcm-bridge\"\n")
        conf.write("        template(t_dcm_marker)\n")
        conf.write("        flags(no-multi-line)\n")
        conf.write("        log-fifo-size(1000)\n")
        conf.write("    );\n")
        conf.write("};\n")
        conf.write("log { source(s_journald); destination(d_unified); };\n")
        conf.write("log { source(s_journald); parser(p_dcm_markers); filter(f_dcm_marker); destination(d_dcm_bridge); };\n")
        conf.close()
}
